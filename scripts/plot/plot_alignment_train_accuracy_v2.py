...
def main():
    ...
    parser.add_argument('--output', type=str, default='plots/alignment_train_accuracy_v2.png',
    ...
    if os.path.exists(seed_path):
        ...
        experiments.append({
            ...
            'is_independent': True,
            'is_same_init': False,
            'show_original': True  # Flag to show the original point
        })
    else:
    ...
    if lmc_exps:
        ...
        y_org = [100 - e['org_train_min'] for e in lmc_exps]  # Convert to barrier
        ax.scatter(x_org, y_org, c=org_color, marker='o', s=100,
                   label='Original (shared training)', zorder=5)

        # Plot LMC experiments - Recovered
        x_rec = [e['w0_w1_recovered'] for e in lmc_exps]
        y_rec = [100 - e['rec_train_min'] for e in lmc_exps]  # Convert to barrier
        ax.scatter(x_rec, y_rec, c=rec_color, marker='o', s=100,
                   label='Recovered (shared training)', zorder=5)

        # Add labels and arrows connecting original to recovered
        for e in lmc_exps:
            org_barrier = 100 - e['org_train_min']
            rec_barrier = 100 - e['rec_train_min']
            # Label original point
            ax.annotate(e['name'], (e['w0_w1'], org_barrier),
                        textcoords="offset points", xytext=(-5, 8), fontsize=8,
                        color=org_color)
            # Draw arrow from original to recovered
            ax.annotate('', xy=(e['w0_w1_recovered'], rec_barrier),
                        xytext=(e['w0_w1'], org_barrier),
                        arrowprops=dict(arrowstyle='->', color='gray', alpha=0.7, lw=1.5))

    # Plot independent experiments (different init) - original and recovered points with arrow
    ...
    if ind_diff_init_exps:
        for e in ind_diff_init_exps:
            org_barrier = 100 - e['org_train_min']
            rec_barrier = 100 - e['rec_train_min']
            # Plot original point (same position as 0/200 LMC original)
            if e.get('show_original', False):
                label_orig = 'Original (diff init)' if first_diff_init_orig else None
                first_diff_init_orig = False
                ax.scatter([e['w0_w1']], [org_barrier], c=ind_diff_init_color, marker='D', s=150,
                           edgecolors='black', linewidths=1.5, alpha=0.5,
                           label=label_orig, zorder=6)
                # Draw arrow from original to recovered
                ax.annotate('', xy=(e['w0_w1_recovered'], rec_barrier),
                            xytext=(e['w0_w1'], org_barrier),
                            arrowprops=dict(arrowstyle='->', color=ind_diff_init_color, alpha=0.7, lw=2))

            # Plot recovered point
            label_rec = 'Recovered (diff init)' if first_diff_init else None
            first_diff_init = False
            ax.scatter([e['w0_w1_recovered']], [rec_barrier], c=ind_diff_init_color, marker='D', s=150,
                       edgecolors='black', linewidths=1.5,
                       label=label_rec, zorder=6)

    # Styling
    ax.set_xlabel('L2 Distance', fontsize=12)
    ax.set_ylabel('Train Barrier (%)', fontsize=12)
