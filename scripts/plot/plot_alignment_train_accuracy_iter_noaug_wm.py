...
def main() -> None:
        ...
        experiments.append(
            {
                ...
                "rec_rel_barrier_mean": rec_y_mean,
                "rec_rel_barrier_std": rec_y_std,
    ...
    y_org = [e["org_rel_barrier"] for e in experiments]
    ...
    y_rec = [e["rec_rel_barrier_mean"] for e in experiments]
    xerr = [e["w0_w1_recovered_std"] for e in experiments]
    yerr = [e["rec_rel_barrier_std"] for e in experiments]
    ...
    for e in experiments:
        ...
        ax.annotate(
            "",
            xy=(e["w0_w1_recovered_mean"], e["rec_rel_barrier_mean"]),
            ...
            writer.writerow(
                {
                    ...
                    "w0_w1": e["w0_w1"],
                    "org_rel_barrier": e["org_rel_barrier"],
                    "w0_w1_recovered_mean": e["w0_w1_recovered_mean"],
                    "w0_w1_recovered_std": e["w0_w1_recovered_std"],
                    "rec_rel_barrier_mean": e["rec_rel_barrier_mean"],
                    "rec_rel_barrier_std": e["rec_rel_barrier_std"],
                    "num_runs": e["num_runs"],
                }
            )
    ...
    print("\n" + "=" * 120)
    print("ITER-SPLIT CONTROLLED BENCHMARK SUMMARY (TRAIN RELATIVE BARRIER, AGGREGATED OVER WM SEEDS)")
    print("=" * 120)
    print(
        f"{'split':>8} {'orig_x':>10} {'orig_rel':>12} {'rec_x_mean':>12} {'rec_x_std':>11} "
        f"{'rec_rel_mean':>13} {'rec_rel_std':>12} {'n':>4}"
    )
    print("-" * 120)
    for e in experiments:
        print(
            f"{e['split_iter']:>8d} "
            f"{e['w0_w1']:>10.2f} "
            f"{e['org_rel_barrier']:>12.6f} "
            f"{e['w0_w1_recovered_mean']:>12.2f} "
            f"{e['w0_w1_recovered_std']:>11.4f} "
            f"{e['rec_rel_barrier_mean']:>13.6f} "
            f"{e['rec_rel_barrier_std']:>12.6f} "
            f"{e['num_runs']:>4d}"
        )
