"""Reusable shared-training implementation for the LMC benchmark.

It mirrors the retained pytorch-vgg recipe while preserving optimizer state in
checkpoints so shared-to-split resume boundaries remain scientifically correct.
"""
...
def build_loaders(data_root: str, batch_size: int, workers: int) -> tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    return core_data.build_cifar10_pytorch_vgg_train_test_loaders(data_root, batch_size, workers)
