...
from mode_connectivity.core import setup, checkpoint, models, data, output
from mode_connectivity.evaluation import interpolation, evaluate
...
def evaluate_linear(args):
    ...
    # Setup
    setup.EvalSetup.add_external_path()
    device = setup.EvalSetup.get_device()
    loaders, num_classes = setup.EvalSetup.load_data(
        args.dataset, args.data_path, args.batch_size,
        args.num_workers, args.transform, args.use_test,
        shuffle_train=False
    )
    architecture = setup.EvalSetup.get_architecture(args.model)
    ...
    # Create model
    model = setup.EvalSetup.create_standard_model(architecture, num_classes, device)
    ...
    def linear_interp(t):
        return interpolation.Interpolator.linear(w1, w2, t)
    ...
    for i, t in enumerate(ts):
        ...
        l2_norms[i] = interpolation.Interpolator.compute_l2_norm(weights)

    # Save results
    output_path = output.ResultSaver.get_standard_output_path(args.dir, 'linear.npz')
    output.ResultSaver.save_with_l2(
        output_path,
        results['ts'],
...
def evaluate_curve(args):
    ...
    # Create curve model
    curve_model = setup.EvalSetup.create_curve_model(
    ...
    # Save results
    output_path = output.ResultSaver.get_standard_output_path(args.dir, 'curve.npz')
    output.ResultSaver.save_from_dict(output_path, results)
...
def evaluate_symmetry(args):
    ...
    def symmetry_interp(t):
        return interpolation.Interpolator.symmetry_plane(w1, theta, w2, t)
    ...
    # Save results
    output_path = output.ResultSaver.get_standard_output_path(args.dir, 'symmetry_plane.npz')
    output.ResultSaver.save_from_dict(output_path, results)
...
def evaluate_comparison(args):
    ...
    # Save results
    output_path = output.ResultSaver.get_standard_output_path(args.dir, 'comparison.npz')
    output.ResultSaver.save_comparison(output_path, ts, linear_results, symplane_results)
