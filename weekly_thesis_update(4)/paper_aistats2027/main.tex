...
\begin{abstract}
The one-basin modulo permutation hypothesis, introduced by Entezari et al. and
later operationalized by Git Re-Basin, proposes that independently trained
neural networks can often be made linearly mode connected by permuting hidden
units.  We test the limits of this explanation.  Exhaustive enumeration on
minimal-width XOR networks gives a counterexample among gradient-trained
solutions: under both full-batch gradient descent and minibatch stochastic
gradient descent, successful endpoints can remain linearly incompatible under
every hidden-unit permutation.  Permutation is also not the complete exact
symmetry relevant to interpolation.  Exploiting positive ReLU scaling lowers
the barrier beyond permutation alignment throughout our controlled XOR study
and across all evaluated VGG/CIFAR-10 architectures.  Nonlinear path
experiments further reveal a sharp width-dependent change in connectivity,
showing that architectural redundancy matters independently of symmetry.
Finally, a controlled branching experiment identifies an early transition in
training geometry: branches that share training through this transition remain
linearly compatible after symmetry alignment, whereas branches allowed to
diverge earlier develop persistent aligned barriers.  These findings separate
two mechanisms: symmetry determines which equivalent representatives are
compared, whereas redundancy and training history determine which low-loss
...
For scalable optimization, we relax each hard permutation to
\begin{equation}
...
layer's inputs the compensating $D_\ell^{-1}S_\ell^\top$.  After projecting
$S_\ell$ to a hard assignment, this is exactly function preserving.  During
soft optimization, the transpose is the standard differentiable Sinkhorn
...
GD.  They do not contradict the sufficiently-wide, high-probability one-basin
conjecture.  They also complement the hand-constructed counterexample of
...
\section{Experiment IV: An Early Transition in Training Connectivity}
\label{sec:training-stage}

\paragraph{Setup.}
Two VGG16/CIFAR-10 branches share an initialization and the first $n$ of
$N=200$ epochs, then train independently with different minibatch orders and
augmentations.  A known random channel permutation is applied to one endpoint,
and weight matching attempts to recover compatibility.  The inverse planted
permutation supplies a reference, while repeated layer-update orders measure
search variability.  This design varies shared training history; it is not a
checkpoint-by-checkpoint decomposition of raw, scale, and nonlinear barriers.

\paragraph{Result.}
Figure~\ref{fig:training-stage} plots test-error barrier against endpoint
distance.  Late splits lie in the lower-left: their endpoints remain close,
the reference path has low barrier, and matching reliably removes the planted
mismatch.  Early splits lie farther away and retain larger barriers.

A denser sweep over the first training epochs localizes a sharp transition in
this run.  After alignment, the test-loss barrier is $0.120$ when the branches
share five epochs and train independently for $195$, but falls to $0.001$ for
the $6/194$ split and remains near zero for all later splits.  Thus, sharing one
additional early epoch qualitatively changes the linear geometry of the final
solutions.  Branches separated after this transition remain in a common linear
basin modulo symmetry despite different subsequent minibatches and
augmentations; branches separated earlier develop residual barriers that
permutation alignment does not remove.
...
a broader symmetry orbit, as in Section~\ref{sec:method}.  The result identifies
a transition in linear compatibility; because curved paths were not optimized
for every shared-split pair, it does not by itself establish that the residual
pairs are connected only nonlinearly.
...
\begin{figure*}[t]
    ...
    have lower barriers after alignment; the dense early-epoch sweep discussed
    in the text places the sharp loss-barrier transition between the $5/195$
    and $6/194$ splits.}
    \label{fig:training-stage}
\end{figure*}

\section{Experiment V: VGG/CIFAR-10 Validation}
\label{sec:vgg}
...
form of the one-basin conjecture~\citep{entezari2021role,ainsworth2023git}.
...
a larger matching problem.  The training-history experiment adds a final
caution: comparable Euclidean distances can conceal different barriers, so
distance is not a certificate of functional compatibility.
...
to fixed VGG/CIFAR-10 pairs, and shared-training experiments show that
compatibility also depends on optimization history.  The central distinction
is therefore simple: symmetry determines which representatives we compare;
redundancy determines which paths are available.
...
The evaluated shared-training durations are
$n\in\{0,1,2,3,4,5,6,8,30,80,100\}$ epochs; the figure displays a readable
subset, while the dense early sweep is used to localize the transition reported
in Section~\ref{sec:training-stage}.
